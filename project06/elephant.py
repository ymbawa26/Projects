...
Oct 25th
elephants 1 project
This project will simulate the elephant population in Kruger National Park. it wuill retuen back the total population after N numbernumber opf years. The population will also return back the number of female and male elephants,
 adults, calfs, and senior.

To run this program cd to the file location then type python3 elephant.py probDart (as as argument). Example python3 elephant 0.2
"""
import random
import stats
...
def calcSurvival(parameter_list, population):
    ...
    for items in population:
        ...
        if items[IDXAge] > 60:
            if random.random() < parameter_list[IDXProb_SeniorSurvival]:
                # print("Dfasdfa", parameter_list[IDXProb_SeniorSurvival])
                new_population.append(items)
        elif items[IDXAge] <= 60 and items[IDXAge] >= 2:
           ...
           # if monthsContraceptive is greater than zero
            ...
            # else if monthsPregnant is greater than zero
                ...
                if monthsPregnant >= 22:
                    ...
                    elephant = newElephant(parameter_list, 1)
                    population+= [elephant]
...
def calcResult(parameter_list, population, numculled):
...
def runSimulation(parameter_list):
    ...
    for i in range(parameter_list[IDXNumber_Years]):
        ...
        [population,numCulled] = controlPopulation( parameter_list, population )
        results.append( calcResult( parameter_list, population, numCulled ) )
...
def test():
    ...
    incrementpopulation = []
    for i in range(15):
        incrementpopulation = incrementAge(incrementpopulation)
    for e in population:
        print(e)

def defaultParameters():
    """ This function takes no and arguments and returns back the parameters list"""
    ...
    carryingCapacity = 50
    numYears = 200
    #creating a list for the parameters 
    parameter_list2 = [calvingInt, probDart, juvAge, maxAge, probCalfSurvival, probAdultSurvival, probSeniorSurvival, carryingCapacity, numYears]
    #return the parameter list
    return parameter_list2

def elephantSim(percDart, inputParameter = None):
    """input: The arguments to the function should be the percent darted percDart and a parameter list
    Output: The function should return the carrying capacity minus the average total population of the simulations. """
    #If inputParameters is equal to the value None, then assign to parameters the result of calling your defaultParameters function.
    if inputParameter == None:
       parameters=  defaultParameters()
     #otherwise, assign to parameters the value of inputParameters.
    else:
        #assign to parameters the input argument "inputParameter"
        parameters = inputParameter 
        #assign to the percent darted element of the parameters list the value of the probDart parameter
    parameters[IDXPercentDarted] = percDart


    # print("CHECKPOINT 1")
    #results the result of calling the runSimulation
    results = runSimulation(parameters)

    #Then loop 4 times, each time adding the result of calling runSimulation to the results.
    for i in range(4):
        results = results + runSimulation(parameters)
        #initialzing the population
    final_population = 0
    #loop over the results list and calculate the average total population.
    for stats in results:
        final_population += stats[0]
    Average_populaion = final_population/len(results)
    #return the carrying capacity minus the average total population of the simulations
    return (parameters[IDXCarrying_Capacity] - Average_populaion)
